...
{
    public partial class DatosAdministrativos : Form
    {
        ...
        {
            datatablas = neg.SelectDataTable("select * from PersonalAdministrativo " + condicion);
            bool tabla;
            if (datatablas == null)
            {
                tabla = false;
        ...
        {
            if (!neg.Comprobar("select * from PersonalAdministrativo"))
            {
        ...
        {
            DataTable Cierre = neg.SelectDataTable("select * from PersonalAdministrativo where FechaFinal<DATEADD(d,-1,GETDATE()) and Estado='Activo'");
            if (Cierre != null)
            {
            ...
            List<String> Lista = neg.Listas("select Nombres from PersonalAdministrativo where FechaFinal<DATEADD(day,15,GETDATE()) and Estado='Activo'");
            List<Int32> Lista2 = neg.ListasEnteros("select Orden from PersonalAdministrativo where FechaFinal<DATEADD(day,15,GETDATE()) and Estado='Activo'");
            if (Lista != null)
            {
                for (int i = 0; i < Lista.Count; i++)
                {
                    ContratoDias = neg.ObtenerDatos("select DATEDIFF(day,GETDATE(),FechaFinal) from PersonalAdministrativo where Orden=" + Lista2[i] + "");
                    Contrato += "El contrato de " + Lista[i] + " vence en " + ContratoDias + " días\n";
                }
            }
            else Contrato = null;

            Lista = neg.Listas("select Nombres from PersonalAdministrativo where FechaVacaciones<DATEADD(day,400,GETDATE()) and Estado='Activo'");
            Lista2 = neg.ListasEnteros("select Orden from PersonalAdministrativo where FechaVacaciones<DATEADD(day,400,GETDATE()) and Estado='Activo'");
            if (Lista != null)
            {
                for (int i = 0; i < Lista.Count; i++)
                {
                    VacacionesDias = neg.ObtenerDatos("select DATEDIFF(day,GETDATE(),FechaVacaciones) from PersonalAdministrativo where Orden=" + Lista2[i] + "");
        ...
        public void seleccionarcelda(DataGridViewCellEventArgs e)
        {
            if (modo == 0)
            {
                if (e.RowIndex >= 0 && e.RowIndex < dataGridView1.Rows.Count - 1)
                {
                    ...
                    tb_cargo.Text = row.Cells[12].Value.ToString().Trim();
        ...
        {
            Nuevo();
            tb_codigo.Text = neg.NuevoOrden("PersonalAdministrativo").ToString();
            tb_dni.Focus();
        }
        private void b_modificar_Click(object sender, EventArgs e)
        {
            contar = neg.Contar("PersonalAdministrativo", "Orden='" + tb_codigo.Text + "'");
            anulados = neg.Contar("PersonalAdministrativo", "Orden='" + tb_codigo.Text + "' and Estado='Anulado'");
            if (anulados == 0)
            {
                if (contar != 0)
                {
                    DataTable row = neg.SelectDataTable("select * from PersonalAdministrativo where Orden='" + tb_codigo.Text + "'");
                    ...
                    tb_cargo.Text = row.Rows[0][12].ToString().Trim();
        ...
        {
            contar = neg.Contar(" PersonalAdministrativo ", " Orden = '" + AntiguoOrden + "' and Estado='Anulado'");
            if (contar != 0)
                MessageBox.Show("El registro ya fue eliminado");
            else
            {
                if (neg.EliminarAdministrativo(AntiguoOrden))
                {
        ...
        private void b_guardar_Click(object sender, EventArgs e)
        {
            ...
            if (mensaje.Equals("Los siguientes datos no pueden estar vacios:"))
            {
                ...
                {
                    Valor = neg.GuardarAdministrativo(modo, AntiguoOrden, tb_nombres.Text, tb_apellidos.Text, tb_dni.Text, tb_fijo.Text, tb_direccion.Text, tb_celular.Text, tb_referencia.Text, dateTimePicker1.Value, dateTimePicker2.Value, dateTimePicker3.Value, cb_estado.Text, tb_cargo.Text, tb_salario.Text, cb_seguro.Text, tb_descuento.Text, tb_total.Text);
                    if (Valor == 1)
                    {
        ...
        {
            HistorialDatosAdministrativos pr = new HistorialDatosAdministrativos();
            pr.ShowDialog(this);
        }
        ...
        {
            if (imp.SetupThePrinting(printDocument1, dataGridView1, "Datos del Personal Administrativo", "Datos del Personal Administrativo"))
            {
